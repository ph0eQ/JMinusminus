...
public class Parser {
	...
	private JMember memberDecl(ArrayList<String> mods) {
		...
		if (seeIdentLParen()) {
			...
			if (have(VOID)) {
				...
				String name = scanner.previousToken().image();
				ArrayList<JFormalParameter> params = formalParameters();
				ArrayList<String> thrown = new ArrayList<String>();
				if(have(THROWS)){
					//JVariableDeclaration args = localVariableDeclarationThrowsStatement();
					while(have(IDENTIFIER)){
						thrown.add(scanner.previousToken().image());
						have(COMMA);
					}
	...
	private JSwitchBlock switchBlock() {
		int line = scanner.token().line();
		ArrayList<JStatement> defaultStatements = new ArrayList<JStatement>();
		ArrayList<JBlock> switchBlocks = new ArrayList<JBlock>();
		ArrayList<String> cases = new ArrayList<String>();
		//trace("Should be LCURLY");
		mustBe(LCURLY);
		while (!see(EOF) && !see(DEFAULT)) {
			if(have(CASE)){
				ArrayList<JStatement> statements = new ArrayList<JStatement>();
				//		trace("Should be a case")
				mustBe(IDENTIFIER);
				cases.add(scanner.previousToken().image());
				mustBe(COLON);
				while(!see(EOF) && !see(CASE) && !see(DEFAULT)){
					statements.add(blockStatement());
				}
				switchBlocks.add(new JBlock(line, statements));
		...
		mustBe(COLON);
		cases.add("Default");
		while(!see(EOF) && !see(RCURLY)){
			defaultStatements.add(blockStatement());
		}
	
		mustBe(RCURLY);
		switchBlocks.add(new JBlock(line, defaultStatements));
		
		return new JSwitchBlock(line, cases, switchBlocks);
	}
	...
	private JStatement statement() {
		...
		if (see(LCURLY)) {
			...
			//JStatement init = statement();
			//ArrayList<JVariableDeclarator> init = variableDeclarators(type());
			JVariableDeclaration init = localVariableDeclarationForStatement();
			//if(have(SEMI)){
				...
				return new JForStatement(line, init, term, inc, body);
			//}else {
				//mustBe(COLON);
				//JExpression term = expression();
				//JStatement inc = null;
			...
			JExpression cond = primary();
			mustBe(SEMI);
			return new JThrowStatement(line, cond);

		}else if(have(SWITCH)){
			//JVariableDeclarator condition = variableDeclarator(type());
			//ArrayList<JFormalParameter> conditions = see(LPAREN) ? formalParameters() : null;

			JExpression condition = parExpression();
			JSwitchBlock body = switchBlock();
			...
			ArrayList<JVariableDeclaration> exceptions = new ArrayList<JVariableDeclaration>();
			while(have(CATCH)){
				mustBe(LPAREN);
				JVariableDeclaration exception = localVariableDeclarationCatchStatement();
				mustBe(RPAREN);
				if(see(LCURLY)){
			...
			if(have(FINALLY)){
				JStatement finallyBlock = blockStatement();

				return new JTryCatchFinallyStatement(line, tryBlock, catches, exceptions, finallyBlock);
			...
			JExpression condition = parExpression();
			mustBe(SEMI);
			return new JDoUntilStatement(line, condition, statement);
			}else {
				mustBe(WHILE);
				JExpression condition = parExpression();
				mustBe(SEMI);
	...
	/**
	 * Parse a local variable declaration statement with for-statement.
	 * 
	 * <pre>
	 *   localVariableDeclarationStatement ::= type 
	 *                                           variableDeclarators 
	...
	private JVariableDeclaration localVariableDeclarationForStatement() {
		int line = scanner.token().line();
		ArrayList<String> mods = new ArrayList<String>();
		ArrayList<JVariableDeclarator> vdecls = variableDeclarators(type());
		mustBe(SEMI);
	...
	private JVariableDeclaration localVariableDeclarationCatchStatement() {
		int line = scanner.token().line();
		ArrayList<String> mods = new ArrayList<String>();
		ArrayList<JVariableDeclarator> vdecls = variableDeclarators(type());
		if(have(LPAREN));
	...
	private JVariableDeclaration localVariableDeclarationThrowsStatement() {
		int line = scanner.token().line();
		ArrayList<String> mods = new ArrayList<String>();
		ArrayList<JVariableDeclarator> vdecls = variableDeclarators(type());
	...
	private JExpression assignmentExpression() {
		...
		//JExpression lsh = conditionalExpression();
		if (have(ASSIGN)) {
			...
			return new JURShiftAssignOp(line, lhs, assignmentExpression());
	...
	private JExpression equalityExpression() {
		int line = scanner.token().line();
		boolean more = true;
		//small edit to test out new conditionalExpressions
	...
	private JExpression conditionalExpression() {
		...
		while(more){
			if(have(TERNARY)){
				...
				JExpression alternate = have(COLON) ? expression(): null;
				//mustBe(SEMI);
				lhs = new JTernaryOp(line, lhs, consequent, alternate);
			}else {
				more = false;
			}
		}
	...
	private JExpression additiveExpression() {
		...
		while (more) {
			if (have(MINUS)) {
				...
				lhs = new JBOrOp(line, lhs, multiplicativeExpression());
			}else if (have (BAND)){
				lhs = new JBAndOp(line, lhs, multiplicativeExpression());
			}else if (have (BXOR)){
				lhs = new JBXorOp(line, lhs, multiplicativeExpression());
			} else {
				more = false;
			}
		}
	...
	private JExpression multiplicativeExpression() {
		...
		while (more) {
			if (have(STAR)) {
				...
				lhs = new JRShiftOp(line, lhs, unaryExpression());
			}else if(have(URSHIFT)){
				lhs = new JURShiftOp(line, lhs, unaryExpression());
			}else if(have(LSHIFT)) {
				lhs = new JLShiftOp(line, lhs, unaryExpression());
			}else if(have(MOD)){
