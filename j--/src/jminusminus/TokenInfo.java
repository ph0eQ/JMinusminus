...
enum TokenKind {
    //reserved words
	//now supports all valid reserved words in java 
	...
	DEFAULT("default"), DO("do"), DOUBLE("double"), ELSE("else"), EXTENDS("extends"), FALSE("false"), 
	FINAL("final"), FINALLY("finally"), FLOAT("float"), FOR("for"), GOTO("goto"), IF("if"),
    ...
    //operators
    //now supports all valid operators in java
    TERNARY("?"), ASSIGN("="), EQUAL("=="), LNOT("!"), BNOT("~"), NOT_EQUAL("!="),
    DIV("/"), DIV_ASSIGN("/="), PLUS("+"), PLUS_ASSIGN("+="), INC("++"), 
    MINUS("-"), MINUS_ASSIGN("-="), DEC("--"), STAR("*"), STAR_ASSIGN("*="),
    MOD("%"), MOD_ASSIGN("%="), RSHIFT(">>"), RSHIFT_ASSIGN(">>="), URSHIFT(">>>"),
    URSHIFT_ASSIGN(">>>="), GE(">="), GT(">"), LSHIFT("<<"), LSHIFT_ASSIGN("<<="),
    LE("<="), LT("<"), BXOR("^"), BXOR_ASSIGN("^="), BOR("|"), BOR_ASSIGN("|="),
    LOR("||"), BAND("&"), BAND_ASSIGN("&="), LAND("&&"), UNTIL("until"), 
    
    
    //separators
    LPAREN("("), RPAREN(")"), LCURLY("{"), RCURLY("}"), LBRACK("["),
    RBRACK("]"), SEMI(";"), COMMA(","), DOT("."), COLON(":"), IDENTIFIER("<IDENTIFIER>"),
    ELLIPSIS("..."),
    //literals
    ...
    SHORT_LITERAL("<SHORT_LITERAL>"), BYTE_LITERAL("<BYTE_LITERAL>");
